...
from schemas.queue import QueueCreate, QueueResponse, QueueUpdate
from services import queue_config
...
async def create_queue(
    ...
    _get_instance_or_404(db, instance_id)
    try:
        return queue_config.create_queue(cdr_db, instance_id, data)
...
async def update_queue(
    ...
    try:
        return queue_config.update_queue(cdr_db, instance_id, queue_name, data)
...
async def delete_queue(
    ...
    _get_instance_or_404(db, instance_id)
    try:
        deleted = queue_config.delete_queue(cdr_db, instance_id, queue_name)
    except Exception as e:
        cdr_db.rollback()
        raise HTTPException(status_code=500, detail=str(e)) from e
